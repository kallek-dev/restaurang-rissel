...
import { z } from "zod";
import { listDateExceptions, createDateException } from "@/lib/schedule";

export const dynamic = "force-dynamic";

export async function GET() {
  const exceptions = await listDateExceptions();
...
export async function POST(req: NextRequest) {
  ...
  const exception = await createDateException(parsed.data);
