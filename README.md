...
Next.js 14 (App Router, TypeScript) + Prisma (databas) + Mailjet (mail) +
...
npm install
cp .env.example .env
# fyll i DATABASE_URL (samma Postgres-uppkoppling som produktion funkar
# bra, eller en egen lokal Postgres)
# fyll i MJ_APIKEY_PUBLIC, MJ_APIKEY_PRIVATE och MJ_FROM_EMAIL
# (valfritt lokalt —
# utan dem loggas bara en varning och inga mail skickas)
# fyll i ADMIN_USERNAME och ADMIN_PASSWORD — /admin fungerar inte
...
- Ett **Mailjet**-konto för mail (gratis, ingen egen domän behövs)
...
byggkommandot kör `prisma db push` automatiskt — så när `DATABASE_URL`
...
1. Skapa en gratis Postgres-databas. Enklast på Vercel: i projektet →
   ...
   Neon). Kopplas in automatiskt, ingen `DATABASE_URL` att krångla med
...
2. Om du skapade databasen någon annanstans än via Vercels Storage-flik:
   sätt `DATABASE_URL` manuellt under Project Settings → Environment
   Variables.
...
Ingen egen domän behövs — Mailjet låter dig verifiera en enda
mailadress istället för en hel domän. Gratis upp till 6 000 mail/månad
(200/dag), ingen tidsgräns.

1. Skapa konto på [mailjet.com](https://mailjet.com).
2. Gå till **Account Settings → Senders & Domains → Add a sender
   address**. Fyll i en mailadress ni har tillgång till (t.ex. ett
   fristående Gmail-konto skapat bara för restaurangen) och bekräfta
   via länken som skickas dit.
3. Gå till **Account Settings → API Key Management** → kopiera er
   **API Key** (publik) och **Secret Key** (hemlig) — Mailjet kräver
   båda, till skillnad från många andra tjänster.
4. Sätt `MJ_FROM_EMAIL` till **exakt** samma adress ni verifierade
   (t.ex. `Restaurang Rissel <dinadress@gmail.com>`).

*Värt att veta:* Mailjet avråder från att använda en gratis
Gmail/Outlook-adress som avsändare eftersom det kan öka risken för att
mail hamnar i mottagarens skräppost. Fungerar fint för den här skalan,
men om ni någon gång skaffar en egen domän är det en enkel uppgradering
att verifiera hela domänen istället.
...
2. Lägg till miljövariabler under Project Settings → Environment
   Variables: `MJ_APIKEY_PUBLIC`, `MJ_APIKEY_PRIVATE`, `MJ_FROM_EMAIL`,
   `CRON_SECRET`, `ADMIN_USERNAME`, `ADMIN_PASSWORD`. (`DATABASE_URL`
...
2. Lägg till miljövariabler under Site configuration → Environment
   variables: `DATABASE_URL`, `MJ_APIKEY_PUBLIC`, `MJ_APIKEY_PRIVATE`, `MJ_FROM_EMAIL`,
   `CRON_SECRET`, `ADMIN_USERNAME`, `ADMIN_PASSWORD`.
3. Deploya. Databastabellerna skapas automatiskt som en del av bygget
   (`prisma db push`), inget separat kommando behövs.
...
här projektet använder. Sätt miljövariabeln `CRON_SECRET` — endast
anrop med rätt hemlighet (`Authorization: Bearer <CRON_SECRET>` eller
`?secret=<CRON_SECRET>`) tillåts.
...
GET-anrop:
`https://dinsajt.se/api/cron/reminders?secret=DIN_CRON_SECRET` och
`https://dinsajt.se/api/cron/cleanup?secret=DIN_CRON_SECRET`.

Utan `CRON_SECRET` satt körs endpointerna öppet (funkar, men vem som
...
- **Adminpanelen är lösenordsskyddad** (Basic Auth, se `src/middleware.ts`).
  Sätt `ADMIN_USERNAME` och `ADMIN_PASSWORD` som miljövariabler — utan
  `ADMIN_PASSWORD` är panelen helt avstängd, inte öppen, så systemet
...
- **Integritetspolicy** finns på `/integritetspolicy`, länkad från både
  ...
  dataskyddsombud.
- **Underleverantörer** (Vercel/Netlify, er valda databas, Mailjet) är
...
`ADMIN_USERNAME` och `ADMIN_PASSWORD`. Samma skydd gäller alla
